...
export default function sitemap(): MetadataRoute.Sitemap {
  const routes = ['', '/gallery', '/series', '/who-i-am', '/process', '/commissions', '/contact', '/preview']
  ...
  const artworkPages = AVAILABLE_ARTWORKS.map((artwork) => ({
  ...
  const previewPages = AVAILABLE_ARTWORKS.map((artwork) => ({
