...
function SplashScreen({ onDone }: { onDone: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="splash-sub text-[10px] tracking-[0.32em] uppercase text-text-3 font-mono">
        Fine Art Studio
      </span>
...
export default function HomeClient({ artworks }: { artworks: Artwork[] }) {
  ...
  const handleBuy = useCallback((art: Artwork) => setBuyModal(art), [])
  const handleInquire = useCallback((art: Artwork) => setInqModal(art), [])
  ...
  return (
    <>
      ...
      <div style={{ opacity: splashDone ? 1 : 0, transition: 'opacity 0.7s ease 0.1s' }}>
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              fontWeight: 300, color: 'rgba(255,255,255,0.022)', lineHeight: 1,
          ...
          <div className="absolute inset-0">
            <Image
              ...
              style={{
                ...
                filter: 'brightness(0.38) contrast(1.22) saturate(0.78)',
          ...
          <div
            ...
            style={{
              background:
                'radial-gradient(ellipse 80% 90% at 65% 50%, transparent 15%, rgba(7,6,10,0.65) 100%)',
            }}
            aria-hidden
          />
          <div
            ...
            style={{
              background:
                'linear-gradient(to top, #15131c 0%, rgba(21,19,28,0.52) 38%, transparent 72%)',
            }}
            aria-hidden
          />
          ...
          >
            ...
            >
              The<br />moment<br />
              <em className="italic text-text-2">before</em>
              <br />the flame
            </h1>
            <p className="font-mono text-[13px] text-text-2 leading-[1.85] max-w-[340px] mb-12 font-light tracking-[0.025em]">
              Figurative oil paintings that live in the emotional space between tension and
              release. Dark. Dramatic. Permanent.
            </p>
            <div className="flex gap-3 flex-wrap">
              <Link href="/gallery" className="btn-ember">
                View the Gallery
              </Link>
          ...
          <div
            ...
            aria-hidden="true"
          >
            <span className="font-mono text-[8px] tracking-[0.2em] uppercase text-text-3">
              scroll
            </span>
            <div
              className="w-px h-16"
              style={{
                background: 'linear-gradient(to bottom, transparent, #7a3408)',
        ...
        {/* ─── STATS ────────────────────────────────────────────────────────── */}
        <div
          className="grid grid-cols-3 border-b border-[#38354a] max-sm:grid-cols-1"
          role="list"
        ...
        {/* ─── QUOTE ────────────────────────────────────────────────────────── */}
        <div
          className="relative h-[440px] overflow-hidden flex items-center justify-center"
          aria-hidden="true"
        >
          <Image
            src="/paintings/crimson-study.jpg"
            alt=""
            fill
            className="object-cover"
            style={{
              objectPosition: 'center 25%',
              filter: 'brightness(0.16) contrast(1.4) saturate(0.45)',
            }}
            aria-hidden
            sizes="100vw"
          />
          <Reveal className="relative z-[2] text-center px-[52px] max-w-[820px] mx-auto max-md:px-6">
            ...
            >
              "I paint women the way I experience them. As forces of nature, not objects of
              observation."
            </blockquote>
            <footer
        ...
        <section className="px-[52px] py-[100px] max-md:px-6" aria-label="Series">
          <Reveal>
            <div className="flex justify-between items-baseline mb-[52px]">
              <h2 className="font-serif font-light" style={{ fontSize: 'clamp(28px,4vw,54px)' }}>
                Series
              </h2>
              <Link
                href="/series"
                className="font-mono text-[9.5px] tracking-[0.14em] uppercase text-ember flex items-center gap-2 hover:gap-4 transition-all"
              >
                Explore →
              </Link>
            </div>
          ...
          <Reveal delay={0.1}>
            <div className="grid grid-cols-2 gap-[5px] max-md:grid-cols-1">
              {(
                Object.entries(SERIES) as [string, (typeof SERIES)[keyof typeof SERIES]][]
              ).map(([name, info]) => {
                ...
                return (
                  ...
                  >
                    <Image
                      ...
                      placeholder="blur"
                      blurDataURL={BLUR_PLACEHOLDERS[cover.slug]}
                    />
                    <div
                      className="absolute inset-0 flex flex-col justify-end p-8"
                      style={{
                        background:
                          'linear-gradient(to top, rgba(21,19,28,0.93) 0%, rgba(21,19,28,0.28) 60%, transparent 100%)',
                      }}
                    >
                      <h3
                      ...
                      <p className="font-mono text-[12px] text-text-2 leading-[1.65] max-w-[300px] mb-3.5">
                        {info.desc.slice(0, 90)}…
                      </p>
                      <span className="font-mono text-[9px] tracking-[0.14em] uppercase text-ember">
        ...
        >
          <Reveal>
            <div>
              ...
              <h2
                className="font-serif font-light leading-[1.08]"
                style={{ fontSize: 'clamp(32px,4vw,60px)' }}
              >
                Something made<br />
                <em className="italic text-text-2">only for you.</em>
              </h2>
            </div>
          </Reveal>
          <Reveal delay={0.2}>
            <Link href="/commissions" className="btn-ember flex-shrink-0">
              Start a Commission
            </Link>
          </Reveal>
        </section>
