...
export function Marquee() {
  ...
  return (
    <div className="border-t border-b border-[#2a2622] overflow-hidden flex" aria-hidden="true">
      {[0, 1].map((t) => (
        <div key={t} className={`marquee-track ${t === 1 ? 'marquee-track-2' : ''}`}>
          {doubled.map((item, i) => (
            <span
              key={i}
              className="px-10 py-[17px] text-[10px] tracking-[0.18em] uppercase text-text-3 border-r border-[#2a2622] flex items-center gap-3.5 whitespace-nowrap"
